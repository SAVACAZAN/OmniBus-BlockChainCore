...
// ─── PQ Stage 2 Hard-Fork Heights ────────────────────────────────────────────
//
// Activation height for EDU (coin_type=782, ob_c1_/obc1_, CROSS-RSDPG-128)
// and GOV (coin_type=783, ob_y3_/oby3_, MAYO-3) badges.
//
// Pre-fork nodes MUST reject any TX whose from_address or to_address uses an
// EDU/GOV prefix. This is enforced in Transaction.isValid() via VALID_PREFIXES
// and in Blockchain.validateTransaction() via pqStage2Active(block_height).
//
// Testnet: 100_000 blocks (~1.2 days at 1s/block) — enough to test the fork
//          without waiting for mainnet maturity.
// Mainnet: 200_000 blocks (~2.3 days from genesis; ~58_000 + runway so live
//          testnet (which is already at 58k+) activates immediately on testnet).
//          Change before mainnet launch if needed via governance vote.
pub const PQ_STAGE2_FORK_HEIGHT_MAINNET: u64 = 200_000;
pub const PQ_STAGE2_FORK_HEIGHT_TESTNET: u64 = 100_000;
pub const PQ_STAGE2_FORK_HEIGHT_DEVNET:  u64 = 0;    // always active in devnet/regtest
pub const PQ_STAGE2_FORK_HEIGHT_REGTEST: u64 = 0;

/// Returns true if EDU/GOV PQ Stage 2 badge TXs are allowed at `block_height`
/// on the given network. Consensus MUST call this before accepting a TX that
/// uses scheme .edu_cross_rsdpg / .gov_mayo / .edu_transferable / .gov_transferable.
pub fn pqStage2Active(chain_id: ChainId, block_height: u64) bool {
    const threshold = switch (chain_id) {
        .mainnet => PQ_STAGE2_FORK_HEIGHT_MAINNET,
        .testnet => PQ_STAGE2_FORK_HEIGHT_TESTNET,
        .devnet  => PQ_STAGE2_FORK_HEIGHT_DEVNET,
        .regtest => PQ_STAGE2_FORK_HEIGHT_REGTEST,
    };
