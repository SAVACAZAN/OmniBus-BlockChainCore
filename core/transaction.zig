...
pub const Transaction = struct {
    ...
    /// Hash = SHA256(SHA256(id || from || to || amount || timestamp))
    pub fn calculateHash(self: *const Transaction) [32]u8 {
        var hasher = std.crypto.hash.sha2.Sha256.init(.{});
        var buf: [256]u8 = undefined;
        const str = std.fmt.bufPrint(&buf, "{d}:{s}:{s}:{d}:{d}", .{
            self.id,
            self.from_address,
            self.to_address,
            self.amount,
    ...
    pub fn verify(self: *const Transaction, compressed_pubkey: [33]u8) bool {
        ...
        hexToBytes(self.signature, &sig_bytes) catch return false;
        hexToBytes(self.hash, &hash_bytes) catch return false;
    ...
    pub fn verifyWithHexPubkey(self: *const Transaction, pubkey_hex: []const u8) bool {
        ...
        hexToBytes(pubkey_hex, &pubkey_bytes) catch return false;
