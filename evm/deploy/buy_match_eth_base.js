...
const payload = `{"trader":"${addr}","side":"${side}","pairId":${pairId},"price":${price},"amount":${amount},"nonce":${nonce},"signature":"${Buffer.from(sig.toCompactRawBytes()).toString("hex")}","publicKey":"${Buffer.from(leaf.publicKey).toString("hex")}","evmOrderId":1778887231368406}`;
console.log(payload);
