...
bool ValidatorSet::unregister_validator(const Hash160& address) {
    ...
    spdlog::info("Validator unregistered: {}", omnibus::to_hex(address));
    update_active_set();
    return true;
}
...
bool ValidatorSet::activate_validator(const Hash160& address, u32 height) {
    ...
    spdlog::info("Validator activated: {}", omnibus::to_hex(address));
    update_active_set();
    return true;
}
...
bool ValidatorSet::deactivate_validator(const Hash160& address) {
    ...
    spdlog::info("Validator deactivated: {}", omnibus::to_hex(address));
    update_active_set();
    return true;
}
