...
async function main() {
  ...
  console.log("Funding PQ addresses then running stress-test");
  console.log("=".repeat(70));
  ...
  // Step 2: Wait for confirmation (1 block)
  console.log("Step 2: Waiting for confirmation (12s)...");
  ...
  for (const from of w.slots) {
    for (const to of dests) {
      try {
        // Get nonce
        const acc = await rpc("getbalance", [{ address: from.address }]).catch(() => null);
        const txReq = {
