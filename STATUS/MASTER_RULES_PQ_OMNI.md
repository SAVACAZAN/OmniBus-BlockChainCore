...
| 2026-05-06 | UI `pqKeypairFromSeed` (`frontend/src/api/pq-sign.ts`) was skipping the seed-expansion hash when input length already equaled the algorithm's expected seed length (32B for ML-DSA). That produced a different keypair than the stress test (which always hashed first), stranding test balances on non-canonical addresses. Fix: always apply the canonical expansion (sha256 for 32B, sha512 for 48B, counter-mode SHA-512 for 96B), regardless of input length. UI and stress test now produce identical addresses for the same mnemonic. |
| 2026-05-06 | aweb3 Tauri (`src-tauri/src/wallet.rs`) uses `pqcrypto_*::keypair()` which is OS-RNG (non-deterministic). It does NOT match the UI/chain deterministic derivation. Acceptable for now — desktop app uses Tauri only for advanced flows (sign-with-existing-key); deterministic mnemonic-based derivation goes through the TypeScript UI keystore. To align Tauri later, switch to `liboqs-rs` which exposes `keypair_from_seed`. Tracked but not blocking. |
