// OEP-1 131/150 | path=src/guardian.cpp | proj=omnibus-node-cpp | run=2026-06-01-cpp-v1
...
bool Guardian::request_2fa(const Hash160& account, const Hash160& guardian) {
    ...
    spdlog::info("2FA requested for account {} from guardian {}", account.data(), guardian.data());
    return true;
}
...
bool Guardian::approve_2fa(const Hash160& account, const Hash160& guardian, const std::vector<u8>& signature) {
    ...
    for (auto& auth : it->second) {
        if (auth.guardian == guardian && !auth.approved) {
            ...
            auth.approved = true;
            spdlog::info("2FA approved for account {}", account.data());
            return true;
        }
    }
