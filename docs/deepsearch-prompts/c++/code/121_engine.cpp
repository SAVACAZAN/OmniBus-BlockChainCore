// OEP-1 117/150 | path=src/mining/pool.cpp | proj=omnibus-node-cpp | run=2026-06-01-cpp-v1
...
bool MiningPool::submit_share(const MiningShare& share) {
    ...
    spdlog::debug("Share submitted by miner {}", share.miner.data());
    return true;
}
...
void MiningPool::distribute_rewards(const consensus::Block& block) {
    ...
    for (auto& [addr, stats] : miners_) {
        ...
        spdlog::debug("Miner {} earned reward: {}", addr.data(), reward);
    }
}
