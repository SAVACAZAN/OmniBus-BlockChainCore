// OEP-1 129/150 | path=src/agents/manager.cpp | proj=omnibus-node-cpp | run=2026-06-01-cpp-v1
...
bool AgentManager::register_agent(const Hash160& agent, const std::vector<std::string>& caps) {
    ...
    spdlog::info("Agent registered: {}", agent.data());
    return true;
}
...
bool AgentManager::update_tier(const Hash160& agent) {
    ...
    if (new_tier != it->second.tier) {
        ...
        spdlog::info("Agent {} upgraded to tier {}", agent.data(), static_cast<int>(new_tier));
    }
    return true;
}
