// OEP-1 113/150 | path=src/validator/staking.cpp | proj=omnibus-node-cpp | run=2026-06-01-cpp-v1
...
bool StakingManager::stake(const Stake& stake) {
    ...
    update_tiers();
    spdlog::info("Stake added: {} -> {} amount={}", stake.staker.data(), stake.validator.data(), stake.amount);
    return true;
}
...
bool StakingManager::unstake(const Hash160& staker, const Hash160& validator, u64 amount) {
    ...
    update_tiers();
    spdlog::info("Stake removed: {} -> {} amount={}", staker.data(), validator.data(), amount);
    return true;
}
...
bool StakingManager::claim_rewards(const Hash160& staker, const Hash160& validator) {
    ...
    spdlog::info("Rewards claimed for {} from {}", staker.data(), validator.data());
    return true;
}
