// OEP-1 89/150 | path=include/omnibus/rpc/server.hpp | proj=omnibus-node-cpp | run=2026-06-01-cpp-v1
#pragma once
#include <boost/asio.hpp>
#include <nlohmann/json.hpp>
#include <functional>
#include <map>

namespace omnibus::rpc {

using json = nlohmann::json;

class RPCServer {
    boost::asio::io_context io_;
    boost::asio::ip::tcp::acceptor acceptor_;
    std::map<std::string, std::function<json(const json&)>> methods_;
    u16 port_;
    
public:
    explicit RPCServer(u16 port);
    ~RPCServer();
    
    void start();
    void stop();
    void register_method(const std::string& name, std::function<json(const json&)> handler);
    void run();
    
private:
    void do_accept();
    void handle_client(std::shared_ptr<boost::asio::ip::tcp::socket> socket);
    json process_request(const json& req);
    json make_error(int code, const std::string& message, const json& id);
    json make_result(const json& result, const json& id);
};

} // namespace omnibus::rpc