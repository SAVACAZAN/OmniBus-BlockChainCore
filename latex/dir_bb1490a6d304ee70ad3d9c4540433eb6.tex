\doxysection{frontend/\+node\+\_\+modules/\+dom-\/helpers Directory Reference}
\hypertarget{dir_bb1490a6d304ee70ad3d9c4540433eb6}{}\label{dir_bb1490a6d304ee70ad3d9c4540433eb6}\index{frontend/node\_modules/dom-\/helpers Directory Reference@{frontend/node\_modules/dom-\/helpers Directory Reference}}
Directory dependency graph for dom-\/helpers:\+
% FIG 0


\doxysubsection{Detailed Description}
\hypertarget{dir_bb1490a6d304ee70ad3d9c4540433eb6_autotoc_md6469}{}\doxysubsection{\texorpdfstring{dom-\/helpers}{dom-\/helpers}}\label{dir_bb1490a6d304ee70ad3d9c4540433eb6_autotoc_md6469}
tiny modular DOM lib for ie9+\hypertarget{dir_bb1490a6d304ee70ad3d9c4540433eb6_autotoc_md6470}{}\doxysubsection{\texorpdfstring{Install}{Install}}\label{dir_bb1490a6d304ee70ad3d9c4540433eb6_autotoc_md6470}
...
\item {\ttfamily position(element, \+[offset\+Parent]\+}:\+ return "{}offset"{} of the node to its offset\+Parent, optionally you can specify the offset parent if different than the "{}real"{} one
\item {\ttfamily scroll\+Top(element, \+[value]\+)}
\item {\ttfamily scroll\+Left(element, \+[value]\+)}
...
\item {\ttfamily transition\+End(node, handler, \+[duration]\+, \+[padding]\+)} listens for transition end, and ensures that the handler if called even if the transition fails to fire its end event. Will attempt to read duration from the element, otherwise one can be provided
\item {\ttfamily add\+Event\+Listener(node, event\+Name, handler, \+[options]\+)}:\+
\item {\ttfamily remove\+Event\+Listener(node, event\+Name, handler, \+[options]\+)}:\+
\item {\ttfamily listen(node, event\+Name, handler, \+[options]\+)}:\+ wraps {\ttfamily add\+Eventlistener} and returns a function that calls {\ttfamily remove\+Event\+Listener} for you
\item {\ttfamily filter(selector, fn)}:\+ returns a function handler that only fires when the target matches or is contained in the selector ex:\+ {\ttfamily on(list, \textquotesingle{}click\textquotesingle{}, filter(\textquotesingle{}li \texorpdfstring{$>$}{>} a\textquotesingle{}, handler))}
\item {\ttfamily request\+Animation\+Frame(cb)} returns an ID for canceling
\item {\ttfamily cancel\+Animation\+Frame(id)}
\item {\ttfamily scrollbar\+Size(\+[recalc]\+)} returns the scrollbar\textquotesingle{}s width size in pixels
\item {\ttfamily scroll\+To(element, \+[scroll\+Parent]\+)} 
\end{DoxyItemize}
\end{DoxyItemize}